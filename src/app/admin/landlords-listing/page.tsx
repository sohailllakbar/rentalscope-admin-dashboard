...
export default function LandlordsListingPage() {
  ...
  useEffect(() => {
    ...
    const key = `landlords-${currentPage}-${entries}-${debouncedSearch}`;
    ...
    const fetchLandlords = async () => {
      try {
        ...
        let url = `${BASE_URL}/api/admin/landlords`;
        ...
        if (!json.success || !Array.isArray(json.data?.landlords)) {
  ...
  }, [debouncedSearch, currentPage, entries]);
  ...
  const filteredLandlords = useMemo(() => {
    ...
    return landlords.filter((l) =>
      [l.name, l.email, l.phone, l.gender, l.address]
        .join(" ")
        .toLowerCase()
        .includes(term),
    );
  ...
  const paginatedLandlords = useMemo(() => {
    ...
    return filteredLandlords.slice(start, start + entries);
  }, [filteredLandlords, currentPage, entries]);

  /* ================================
      ACTIONS
  ================================= */
  const handleBlockClick = (landlord: Landlord) => {
  ...
  const handleConfirmBlock = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_URL}/api/blocks/landlord/block/${selectedLandlord.id}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!res.ok || !result.success) {
        throw new Error(result.message || "Failed to block landlord");
      }

      setLandlords((prev) => prev.filter((l) => l.id !== selectedLandlord.id));

      clearLandlordCache(); // ✅ invalidate cache

      toast.success("Landlord blocked successfully");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#F5F6FA] pt-2 pr-6">
      <PageHeader title="Landlords Listing" />
            ...
            <tbody className="divide-y divide-gray-200">
              {paginatedLandlords.map((landlord, index) => (
                <TableRow
                  ...
                  index={index}
                  onAction={(row, action) => {
                    if (action === "block") handleBlockClick(row);
                  }}
                />
              ))}
